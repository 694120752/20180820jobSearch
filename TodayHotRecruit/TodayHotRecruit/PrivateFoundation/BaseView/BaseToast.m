...
@interface BaseToastView : UIView<CAAnimationDelegate>
...
@interface BaseToast ()
@property (nonatomic,strong) BaseToastView *toastView;
@end

@implementation BaseToast

+ (BaseToast *)sharedToast {
    static dispatch_once_t once;
    static BaseToast *obj = nil;
    dispatch_once(&once, ^{
        obj = [[BaseToast alloc] init];
    });
    return obj;
}

- (BaseToastView *)toastView {
    if (nil == _toastView) {
        _toastView = [[BaseToastView alloc] initWithString:nil];
    }
    return _toastView;
}
...
+ (void)toast:(NSString *)toastString view:(UIView *)fromView {
    ...
    dispatch_async(dispatch_get_main_queue(), ^{
        ...
        if (![fromView.subviews containsObject:[self sharedToast].toastView]) {
            ...
            // layout
            BaseToastView *toastView = [self sharedToast].toastView;
