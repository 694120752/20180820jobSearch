...
- (void)createSingleLine {
    _singleLine = [[UILabel alloc] initWithFrame:CGRectMake(10, _topHeight-2, [_buttonArr[_selectInt] frame].size.width, 2)];
...
- (void)scrollViewDidScroll:(UIScrollView *)scrollView {
    ...
    CGFloat calcNewWid = [newWid floatValue] - [_buttonArr[_selectInt] frame].size.width;
    
    CGFloat scale = (right < 0 ? -right: right) / self.width;
    _singleLine.frame = CGRectMake(scale * calcNewX + [_buttonArr[_selectInt] left], _singleLine.top, [_buttonArr[_selectInt] frame].size.width + scale *calcNewWid, _singleLine.height);
