...
//    _explainLabel.text = @"添加图片不超过9张，文字备注不超过300字";
    ...
    [_submitBtn setBackgroundColor:CommonBlue];
    //[UIColor colorWithRed:243.0/255.0 green:60.0/255.0 blue:62.0/255.0 alpha:1.0]
...
- (void)submitToServer{
    ...
    // content
    NSString *content = _noteTextView.text;
    
    for (UIImage *smallImg in smallImageArray) {
        NSData *smallImgData = UIImageJPEGRepresentation(smallImg, 0.8);
        [smallImageDataArray addObject:smallImgData];
    }
    
    NSMutableArray *filePaths = [NSMutableArray array];
    // 发布
    if (!IsArrEmpty(smallImageDataArray)) {
        // 有图就先上传图
        
        dispatch_queue_t disqueue =  dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0);
        dispatch_group_t imageGroup = dispatch_group_create();
        
        dispatch_group_async(imageGroup, disqueue, ^{
            for (NSData* imageData in smallImageDataArray) {
                dispatch_group_enter(imageGroup);
                [THRUploadFile upLoadFileWithData:@[imageData] andTitleArray:nil UploadFailedReason:^(NSString *reasonStr, THRUploadFailedReason reason) {
                    // 失败不做处理
                    dispatch_group_leave(imageGroup);
                } UploadProgressBlock:nil UploadSuccessBlock:^(NSString *filePath) {
                    dispatch_group_leave(imageGroup);
        ...
        dispatch_group_notify(imageGroup, disqueue, ^{
            // 上传 文字
            __weak typeof(self)weakSelf = self;
            [[[THRRequestManager manager] setDefaultHeader] POST:[HTTP stringByAppendingString:@"/forum/add"] parameters:@{@"content":content,@"fileList":filePaths,@"categoryID":self.categoryId} progress:nil success:^(NSURLSessionDataTask * _Nonnull task, id  _Nullable responseObject) {
                DESC
        ...
        // 上传 文字
        __weak typeof(self)weakSelf = self;
        [[[THRRequestManager manager] setDefaultHeader] POST:[HTTP stringByAppendingString:@"/forum/add"] parameters:@{@"content":content,@"fileList":@[],@"categoryID":self.categoryId} progress:nil success:^(NSURLSessionDataTask * _Nonnull task, id  _Nullable responseObject) {
            DESC
            if ([desc isEqualToString:@"success"]) {
                UIAlertController *alertController = [UIAlertController alertControllerWithTitle:nil message:@"发布成功!" preferredStyle:UIAlertControllerStyleAlert];
                UIAlertAction *actionCacel = [UIAlertAction actionWithTitle:@"OK" style:UIAlertActionStyleDefault handler:^(UIAlertAction * _Nonnull action) {
